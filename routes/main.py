...
def dashboard_stats():
    ...
    document_count = Document.query.filter(Document.issue_date >= first_day_of_month).count()
    
    # Count active projects
    project_count = Project.query.filter(Project.status.in_(['Planned', 'Ongoing'])).count()

    # Count blotter records (e.g., 'Pending' or 'Under Investigation')
    blotter_count = Blotter.query.filter(Blotter.status.in_(['Pending', 'Under Investigation', 'Open'])).count()
    ...
    return jsonify({
        ...
        'blotters': blotter_count # Add blotter count
    })
...
def report_blotter():
    ...
    if form.validate_on_submit():
        try:
            ...
            # Ensure incident_date is not None by using today's date as fallback
            ...
            # Make sure to set the complainant name to the current user's name or username
            # This ensures we can find it later based on the user's identity
            if current_user.is_authenticated:
                ...
                # Also try to associate with resident record if possible
                if hasattr(current_user, 'email'):
